...
_HOUSE_PREFIX = re.compile(
    # Сокращение «стр» допустимо только как отдельное слово. Без границы оно
    # отрезало первые три буквы у «Строителей», после чего fuzzy-поиск мог
    # выбрать совершенно другую улицу.
    r"^(?:д\.?|дом|стр\.?|строение)(?=\s|\d|$)\s*",
    re.IGNORECASE,
)
