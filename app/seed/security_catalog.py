...
MODULE_FIELDS: dict[str, list[tuple[str, str, int]]] = {
    "requests": [
        ...
        ("address", "Адрес", 40),
        ("latitude", "Широта", 50),
        ("longitude", "Долгота", 60),
        ("phone", "Телефон", 70),
        ("applicant_name", "ФИО заявителя", 80),
        ("priority", "Приоритет", 90),
        ("status_id", "Статус", 100),
        ("responsible_id", "Ответственный", 110),
        ("executor_id", "Исполнитель", 120),
        ("created_at", "Дата создания", 130),
    ],
    "projects": [
        ("code", "Код", 10),
        ("name", "Название", 20),
