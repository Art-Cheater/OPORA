...
SYSTEM_MODULES = [
    ...
    ("waybills", "Работа по заявкам", "map", 13, "Рабочее место мастера: карта, план и маршрут"),
    ...
    ("contracts", "Контракты", "file-earmark-text", 30, "Контракты из ЕИС и закупок"),
    ("contractors", "Подрядчики", "building", 32, "Справочник подрядчиков"),
    ("agreements", "Договора на опоры", "broadcast", 33, "Договора на размещение оборудования на опорах НО"),
    ...
    ("wallpapers", "Обои", "image", 95, "Каталог фонов интерфейса"),
...
MODULE_FIELDS: dict[str, list[tuple[str, str, int]]] = {
    "requests": [
        ...
        ("repeat_count", "Сколько раз звонили", 140),
    ...
    "objects": [
        ...
        ("kind_comment", "Комментарий к типу объекта", 19),
        ("address", "Адрес", 20),
    ...
    "projects": [
        ...
        ("cable_meters", "Кабель, метры (план)", 105),
        ...
        ("cable_meters_fact", "Кабель, метры (факт)", 145),
    ...
    "agreements": [
        ...
        ("number", "Номер", 20),
        ("subject", "Предмет", 30),
        ("customer_name", "Заказчик", 40),
        ("customer_inn", "ИНН заказчика", 50),
        ("period_from", "Период с", 60),
        ("period_to", "Период по", 70),
        ("source_filename", "Файл", 80),
        ("sites", "Площадки / опоры", 90),
    ],
    "inquiries": [
        ("subject", "Тема", 10),
        ("from_email", "От кого", 20),
        ("from_name", "Имя отправителя", 30),
        ("received_at", "Дата письма", 40),
        ("status", "Статус", 50),
        ("assigned_to", "Исполнитель", 60),
        ("body_text", "Текст", 70),
        ("attachment_count", "Вложения", 80),
    ],
    "eis": [
        ("status", "Статус прогона", 10),
        ("started_at", "Начало", 20),
        ("finished_at", "Окончание", 30),
        ("trigger", "Запуск", 40),
        ("events", "События импорта", 50),
    ],
    "reports": [
        ("period", "Период", 10),
        ("date_from", "Дата с", 20),
        ("date_to", "Дата по", 30),
        ("module", "Раздел отчёта", 40),
    ],
    "materials": [
        ("name", "Наименование", 10),
        ("quantity", "Количество", 20),
        ("unit", "Единица", 30),
    ],
    "messenger": [
    ...
    "audit": [
        ...
        ("action", "Действие", 30),
        ("entity_type", "Тип сущности", 40),
    ],
    "roles": [
        ("code", "Код", 10),
        ("name", "Название", 20),
        ("description", "Описание", 30),
        ("permission_ids", "Права модулей", 40),
        ("field_rules", "Права полей", 50),
    ],
}
