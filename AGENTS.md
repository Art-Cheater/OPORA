...
27. При production-изменении статики обновлять `app/release.py`, если нужен
    новый cache key.
28. На сервере единственная команда установки — `cd /opt/opora && sudo bash
    scripts/deploy.sh`. Миграции, справочники, Docker-сервисы и импорты
    встраивать в этот скрипт. Не оставлять ручные `flask` / `cp` / recreate
    как обязательный шаг после деплоя.
