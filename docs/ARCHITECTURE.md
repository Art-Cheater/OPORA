...
Имена фаз CON9–CON11 задаёт сохранённый `Device.phase_input_map`: source `U2`/`U3`, bit `0..7`, `active_level` 0/1 и флаг `confirmed`. Несколько пинов могут ссылаться на один bit. Неподтверждённый pin в UI — «Не настроено». Переходы битов пишутся в `actual.raw_bit_changes` при очередном STATE. Физическое соответствие pin разъёма биту U2/U3 в файлах `Board/` не доказано.
