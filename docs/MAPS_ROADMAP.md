...
Документ фиксирует картографический стек `MapLibre GL JS + справочник адресов + Valhalla`.
Заявки, дефекты, планы работ, объекты, IRZ и договоры используют MapLibre.
Общий слой описан в `docs/GEO_MAPS.md`, адреса — в `docs/GEO_ADDRESS_DIRECTORY.md`,
подъезды — в `docs/GEO_ENTRANCES.md`, маршруты — в `docs/GEO_ROUTING_ARCHITECTURE.md`.
...
карте». Клик по карте MapLibre заполняет координаты и выставляет
`coordinates_source=manual`. Рядом показывается адрес, найденный через
`GET /api/geo/reverse`; в поле адреса он попадает только по кнопке
«Подставить найденный адрес». Изменение адреса предупреждает проверить точку
и не затирает ручные координаты.
...
multi-point для одной заявки хранится отдельной migration `048_work_map_points`.

## Multi-point адреса

Migration `048_work_map_points` добавляет `work_map_points`: несколько точек
могут принадлежать одной Request или Defect. Первичная точка синхронизируется с
существующими `latitude/longitude`, поэтому маршрут по-прежнему имеет одну
остановку на работу. Ручная точка остаётся primary и repair её не заменяет.
`map.json` отдаёт отдельную feature для каждой точки, но все feature ведут к
одной detail-странице.
