...
Сейчас маршрутизация отложена и в production не включается. Этот файл —
заметка на следующий этап, не инструкция текущего деплоя. Контейнер Valhalla
не ставить, PBF для графа не скачивать, `deploy.sh` не менять.
...
маршрутов, а маршрут не подменяется прямой линией. Кнопки «Построить маршрут»
в интерфейсе нет, пока этап маршрутов не открыт заново.
...
Скрипт проверяет PBF, создаёт каталог данных и перезапускает только `valhalla`,
чтобы новый PBF гарантированно был замечен контейнером.
...
Команда печатает provider и адрес. При пустом URL она пишет
`Disabled / not in current scope` и завершается без ошибки. Пока кнопки
маршрута в интерфейсе нет: план мастера показывает только точки.
